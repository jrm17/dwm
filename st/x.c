...
void xinit(int cols, int rows)
{
	...
	xw.xic = XCreateIC(xw.xim, XNInputStyle, XIMPreeditNothing
			| XIMStatusNothing, XNClientWindow, xw.win, XNFocusWindow, xw.win, NULL);
	if (xw.xic == NULL)
...
void run(void)
{
	...
	do {
		...
		// This XFilterEvent call is required because of XOpenIM. It filters
		// the key event and some client message for the input method as well.
			...
			if (!FD_ISSET(ttyfd, &rfd) && !FD_ISSET(xfd, &rfd)) {
				if (blinkset) {
					if (TIMEDIFF(now, lastblink) > blinktimeout)
						...
						drawtimeout.tv_nsec = (1E6 * (blinktimeout - TIMEDIFF(now, lastblink)));
