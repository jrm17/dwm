		...
		if (priv) {
			switch (*args) {
				...
				case 1005: // UTF-8 mouse mode; will confuse
					// applications not supporting UTF-8 and luit.
			...
			switch (par) {
				...
				case 104: /* color reset, here p = NULL */
					...
					if (xsetcolorname(j, p))
						...
						redraw(); // TODO if defaultbg color is changed, borders are dirty
					return;
			}
			break;
	...
	switch (ascii) {
		...
		case '\a':   /* BEL */
			if (term.esc & ESC_STR_END)
				strhandle(); // backwards compatibility to xterm
			else
				xbell();
			break;
		case '\033': /* ESC */
...
void tputc(Rune u)
{
	...
	// receives a ESC, a SUB, a ST or any other C1 control character.
	if (term.esc & ESC_STR) {
