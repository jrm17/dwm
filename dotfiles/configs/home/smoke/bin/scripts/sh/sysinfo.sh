...
machine_wifi_card=`lspci | grep -m 1 "Wireless" | cut -d: -f3 |  sed 's/^[ \t]*//'`
machine_audio_controller=`lspci | grep -m 1 "Audio" | cut -d: -f3 |  sed 's/^[ \t]*//'`
arch_modules=`grep "MODULES=" /etc/rc.conf `
...
wlan0info=`ifconfig wlan0 | grep "inet addr:" | sed 's/inet addr/Local IP/g' | sed 's/^[ \t]*//;s/[ \t]*$//'`
...
echo " "
echo "eth0: $eth0info"
echo "wlan0: $wlan0info"
echo "ETHERNET CARD: $machine_eth_card"
echo "WIRELESS CARD: $machine_wifi_card"
