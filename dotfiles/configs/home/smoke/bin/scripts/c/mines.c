...
//
// compile with: gcc -lncurses -Wall -std=c99 mines.c -o mines
//
// then it can be run like
//
// mines [height] [width]
//
// Your terminal needs to support mouse input, and at least 2*width + 2 columns wide.
// Left button clears a cell, right button toggles mine mark, middle button on a cleared
// cell clears all neighboring cells (or blow up if there are unmarked mines).
//
// When mine count drops to zero, click "claim victory" to win the game or blow up.
