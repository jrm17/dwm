/* Compile with: gcc -o keyev keyev.c -lX11 -lXtst */
...
void usage(int exit_status, char *program_name)
{
	...
	fprintf(stderr, " XK_ prefix from each name. Each KeySym may be modified by prepending\n");
	fprintf(stderr, " either of ALT-, META-, CTRL- or SHIFT-.\n");
	exit(exit_status);
}
...
int main (int argc, char **argv)
{
	...
	while (arg < argc - 1) {
		if (!strcmp("-p", argv[arg])) {
			arg++;
			printonly = 1;
		} else if (!strcmp("-d", argv[arg]) && arg < argc - 2) {
			arg++;
			delay = atoi(argv[arg++]);
	...
	if (!strncasecmp(argv[arg], "ALT-", 4)) {
		mod = 3;
		memmove(argv[arg], argv[arg] + 4, strlen(argv[arg]) - 3);
	}

	if (!strncasecmp(argv[arg], "META-", 4)) {
		mod = 1;
		memmove(argv[arg], argv[arg] + 5, strlen(argv[arg]) - 4);
	}

	if (!strncasecmp(argv[arg], "CTRL-", 5)) {
		mod = 2;
		memmove(argv[arg], argv[arg] + 5, strlen(argv[arg]) - 4);
	}

	if (!strncasecmp(argv[arg], "SHIFT-", 6)) {
		mod = 4;
		memmove(argv[arg], argv[arg] + 6, strlen(argv[arg]) - 5);
	}
	...
	if (printonly) {
		printf(" key '%s' -> %lx [mod:%s]\n",
				argv[arg],
				symbol,
				modifier[mod]);
	} else
		xkey_send(symbol, mod);
